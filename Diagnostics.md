...
| SAV0005 | ❌ Error | The type containing the `[StyledProperty]` property has an explicit base type that is not derived from `AvaloniaObject`, so `GetValue` and `SetValue` are not available. A type with no explicit base type is not checked, because the base type can be declared in another partial declaration such as one generated from XAML | Derive the containing type from `AvaloniaObject` |
